...
SWIFT_CC(swift)
SWIFT_EXPORT_FROM(swiftDistributed)
void swift_distributed_trace_encode_arguments_end(uint64_t traceID,
                                                  bool success) {
  distributed::trace::distributed_encode_arguments_end(traceID, success);
}
...
SWIFT_CC(swift)
SWIFT_EXPORT_FROM(swiftDistributed)
void swift_distributed_trace_decode_arguments_end(uint64_t traceID,
                                                  size_t argumentCount,
                                                  bool success) {
  distributed::trace::distributed_decode_arguments_end(traceID, argumentCount,
                                                       success);
}
...
SWIFT_CC(swift)
SWIFT_EXPORT_FROM(swiftDistributed)
void swift_distributed_trace_invoke_target_end(uint64_t traceID, bool success) {
  distributed::trace::distributed_invoke_target_end(traceID, success);
}
...
void swift_distributed_trace_invoke_result_handler(
  ...
  distributed::trace::distributed_invoke_result_handler(
      targetActor, targetIdentifier, success);
}
