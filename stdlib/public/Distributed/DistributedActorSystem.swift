...
public protocol DistributedActorSystem<SerializationRequirement>: Sendable {
  ...
  /// Configure the validation macros this actor system enforces, and **inherits** from protocol types
  /// to actual implementation distributed methods.
  ///
  /// This effectively type-level configuration, which needs to specify which macros
  /// should be copied from protocols to implementation distributed methods.
  ///
  /// The configuration needs to spell out types generated by `@DistributedValidatorMacro`
  ///
  /// ### Default behavior
  /// The behavior is to only inherit the built-in `@ValidateRemoteCall` validator.
  /// If an actor system implementation uses other kinds of validation,
  /// it should declare them explicitly in a typealias
  /// and list all the validation macros it supports, e.g.:
  ///
  ///     typealias RemoteCallValidation = DistributedRemoteCallValidation.InheritMacros<EntitlementMacro>
...
extension DistributedActorSystem {
  ...
  public func executeDistributedTarget<Act>(
    ...
    // NOTE: `@Entitlement`/`@ValidateRemoteCall` validation is NOT invoked here.
    // The runtime provides `DistributedValidation.validate(on:target:)` as a
    // primitive; the actor system decides when to call it - typically at the
    // top of `remoteCall`/`remoteCallVoid` on the client side and/or right
    // before delegating to this function on the server side. See
    // `DistributedValidation.validate(on:target:)` for the rationale.
