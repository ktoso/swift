...
// branch below; only the member shapes differ. A `distributed actor` and its
// `ActorSystem: DistributedActorSystem` conformance clause are therefore
// source-portable across the two modes; only the transport-plumbing method
// bodies stay mode-specific.
