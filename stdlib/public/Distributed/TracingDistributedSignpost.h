...
inline void distributed_encode_arguments_end(uint64_t traceId, bool success) {
  if (traceId == 0)
    return;
  ...
  os_signpost_interval_end(
      DistributedRemoteCallsLog, traceId,
      SWIFT_LOG_DISTRIBUTED_OUTBOUND_ENCODE_ARGUMENTS_NAME,
      "success=%{bool}d",
      success);
}
...
inline void distributed_decode_arguments_end(uint64_t traceId,
                                             size_t argumentCount,
                                             bool success) {
  if (traceId == 0)
    return;
...
inline void distributed_invoke_target_end(uint64_t traceId, bool success) {
  if (traceId == 0)
    return;
