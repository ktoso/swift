...
protocol _DistributedValidationMacroImpl {
}

/// Introduces:
/// - TODO: fill in docs
public struct ValidateRemoteCallMacro: _DistributedValidationMacroImpl {
  static var attributeName: String { "@ValidateRemoteCall" }
}

extension ValidateRemoteCallMacro: PeerMacro {
  public static func expansion(
    ...
    if isProtocolRequirementContext(context) {
      ...
      if let arguments = node.arguments?.as(LabeledExprListSyntax.self),
        ...
        throw closureOnProtocolRequirementError(node: node)
      }
      return []
    }
    ...
    // path. Static factories on the validator are typically constrained by
    // `where ActorSystem == ...`, which is what statically enforces the
    // validator's actor system matches the enclosing distributed actor's.
    let validatorExpression =
      "Distributed.RemoteCallValidator<\(enclosing).ActorSystem>(\(argExpression))"
...
extension _DistributedValidationMacroImpl {
  ...
  static func emitValidationPeers(
    ...
    let accessor: DeclSyntax =
      ...
      private static let \(accessorName): Distributed._DistributedValidationAccessor = { outValue, type, hint, reserved in
          let expected: Any.Type = Distributed.RemoteCallValidator<\(raw: enclosingTypeName).ActorSystem>.self
          let requested = type.load(as: Any.Type.self)
          guard requested == expected else { return false }
          let validator: Distributed.RemoteCallValidator<\(raw: enclosingTypeName).ActorSystem> = \(raw: validatorExpression)
          outValue.assumingMemoryBound(to: Distributed.RemoteCallValidator<\(raw: enclosingTypeName).ActorSystem>.self)
  ...
  fileprivate static func sanitizeIdentifier(_ name: String) -> String {
...
func customValidatorExpression(from node: AttributeSyntax) -> String {
...
extension _DistributedValidationMacroImpl {
  ...
  static func diagnoseAttachmentSite(
  ...
  fileprivate static func describeKind(_ declaration: some DeclSyntaxProtocol) -> String {
...
struct DistributedValidationDiagnostic: DiagnosticMessage {
  enum ID: String {
    ...
    case notFoundActorSystemType = "not found ActorSystem type"
    case missingDistributedModifier = "missing distributed modifier"
    case closureOnProtocolRequirementError = "closure on protocol requirement"
  }
...
extension _DistributedValidationMacroImpl {
  ...
  fileprivate static func notDistributedFuncOrVar(
    attributeName: String,
    node: AttributeSyntax,
  ...
  fileprivate static func notFoundActorSystemType(
    attributeName: String,
    node: AttributeSyntax,
    declarationKind: String
  ) -> DiagnosticsError {
    DiagnosticsError(diagnostics: [
      Diagnostic(
        ...
        message: DistributedValidationDiagnostic(
          message:
            "'\(attributeName)' could not locate associated 'ActorSystem' type, ",
          id: .notFoundActorSystemType))
    ])
  }

  fileprivate static func missingDistributedModifier(
    attributeName: String,
    node: AttributeSyntax,
  ...
  static func closureOnProtocolRequirementError(
    node: AttributeSyntax
  ) -> DiagnosticsError {
    DiagnosticsError(diagnostics: [
      Diagnostic(
        ...
        message: DistributedValidationDiagnostic(
          ...
          id: .closureOnProtocolRequirementError))
    ])
  }
}
...
extension _DistributedValidationMacroImpl {
  /// Returns the innermost enclosing distributed-actor / class / struct /
  /// extension name from the macro expansion's lexical context, or `nil` if
  /// none is present. Used by `emitValidationPeers` so the emitted accessor can
  /// spell the actor system type as `<Enclosing>.ActorSystem` - `Self` inside
  /// the emitted `private static let` initializer doesn't resolve to the
  /// enclosing nominal type at typecheck time.
  static func enclosingTypeName(
    ...
    for parent in context.lexicalContext {
      ...
      if let d = parent.as(ExtensionDeclSyntax.self) {
        return d.extendedType.trimmedDescription
      }
    }

    // A distributed func/var MUST be in a distributed actor, so let's throw aggressively here
    throw notFoundActorSystemType(
        attributeName: attributeName,
        node: node,
        declarationKind: describeKind(declaration))
  }
  ...
  static func isProtocolRequirementContext(
    _ context: some MacroExpansionContext
  ) -> Bool {
    for parent in context.lexicalContext {
