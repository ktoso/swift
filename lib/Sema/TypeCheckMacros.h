...
/// `ArgumentList`, then attach it to \p attr via `setArgs`. The preserved text
/// is deliberately NOT cleared: it is read again during cross-module witness
/// synthesis (see `inheritDistributedValidationAttrs`). No-op if the attribute
/// already has an `ArgumentList` or has no preserved text.
