...
void swift::inheritDistributedValidationAttrs(NominalTypeDecl *nominal) {
  ...
  // The set of validation-macro identities this actor system opts into
  // inheriting onto its witnesses, computed LAZILY on the first candidate
  // attribute. Laziness is for correctness, not just speed: this function's
  // main caller is `ExpandPeerMacroRequest`, and resolving the actor-system
  // type re-enters the actor's member expansion and cycles - so an actor with
  // no inheritable validation requirement must never resolve it. Actors that
  // do inherit resolve the system type via a macro-expansion-excluding lookup
  // for the same reason.
  //
  // The system declares
  //   typealias RemoteCallValidation =
  //       DistributedRemoteCallValidation.InheritMacros<...>
  // listing marker types generated by `@DistributedValidatorMacro`; each marker
  // is named `<MacroName>Macro`, so stripping the `Macro` suffix recovers the
  // spelled attribute name matched below (see `attrBaseName`). An explicitly
  // empty list (`InheritMacros< >`) opts out of all inheritance; if the opt-in
  // cannot be resolved we fall back to `@ValidateRemoteCall` only, so a
  // resolution failure never silently drops the core macro (distinct from a
  // resolved-but-empty list, which inherits nothing).
  using LookupFlags = NominalTypeDecl::LookupDirectFlags;
  auto excludeMacroExpansions =
      OptionSet<LookupFlags>() | LookupFlags::ExcludeMacroExpansions;

  // Cycle-safe resolution of the conforming actor's `ActorSystem`: read the
  // explicit `ActorSystem` type member directly (excluding macro expansions, so
  // we don't force this actor's members to expand while one is mid-expansion),
  // else the module-scope `DefaultDistributedActorSystem` typealias. Both avoid
  // the conformance-based accessor, which populates the actor's member table
  // and cycles here. Returns null if neither is found (caller falls back).
  auto actorSystemType = [&]() -> Type {
    for (auto *found : nominal->lookupDirect(ctx.Id_ActorSystem, SourceLoc(),
                                             excludeMacroExpansions)) {
      if (auto *alias = dyn_cast<TypeAliasDecl>(found))
        return alias->getUnderlyingType();
      if (auto *typeDecl = dyn_cast<TypeDecl>(found))
        return typeDecl->getDeclaredInterfaceType();
    }
    // No explicit `ActorSystem`: look for a module-scope
    // `typealias DefaultDistributedActorSystem` (mirrors
    // `deriveDistributedActorType_ActorSystem`).
    auto defaults = TypeChecker::lookupUnqualified(
        nominal->getModuleScopeContext(),
        DeclNameRef(ctx.Id_DefaultDistributedActorSystem), nominal->getLoc());
    for (const auto &found : defaults) {
      if (auto *typeDecl = dyn_cast_or_null<TypeDecl>(found.getValueDecl()))
        return typeDecl->getDeclaredInterfaceType();
    }
    return Type();
  };

  std::optional<llvm::SmallVector<StringRef, 4>> allowedMacroNamesCache;
  auto allowedMacroNames = [&]() -> ArrayRef<StringRef> {
    if (allowedMacroNamesCache)
      return *allowedMacroNamesCache;
    llvm::SmallVector<StringRef, 4> allowed;
    bool resolvedOptIn = false;
    if (nominal->isDistributedActor()) {
      Type systemTy = actorSystemType();
      if (systemTy && !systemTy->hasError() && !systemTy->hasTypeParameter() &&
          !systemTy->hasArchetype()) {
        if (auto *systemNominal = systemTy->getAnyNominal()) {
          Type behavior =
              getDistributedSupportedRemoteCallValidation(systemNominal);
          if (behavior && !behavior->hasError()) {
            resolvedOptIn = true;
            for (auto *marker :
                 getRemoteCallValidationInheritMacroTypes(behavior)) {
              StringRef markerName = marker->getName().str();
              StringRef macroName =
                  markerName.ends_with("Macro")
                      ? markerName.drop_back(StringRef("Macro").size())
                      : markerName;
              if (!macroName.empty())
                allowed.push_back(macroName);
            }
          }
        }
      }
    ...
    if (!resolvedOptIn)
      allowed.push_back("ValidateRemoteCall");
    allowedMacroNamesCache = std::move(allowed);
    return *allowedMacroNamesCache;
  };
  ...
  auto isAllowListed = [&](StringRef name) {
    if (name.empty())
      return false;
    for (StringRef allowed : allowedMacroNames())
      if (allowed == name)
        return true;
    return false;
  };
