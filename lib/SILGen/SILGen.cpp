...
void SILGenModule::emitFunctionDefinition(SILDeclRef constant, SILFunction *f) {
  ...
  if (constant.isDistributedThunk()) {
    fprintf(stderr, "[swift][%s:%d](%s) constant\n", __FILE__, __LINE__, __FUNCTION__);
    constant.dump();
    auto loc = constant.getAsRegularLocation();
    loc.markAutoGenerated();
    auto *dc = loc.getAsDeclContext();
    ...
    if (auto acc = dyn_cast<AccessorDecl>(constant.getFuncDecl())) {
      SILGenFunction(*this, *f, acc->getStorage()->getAccessor(AccessorKind::Get))
          .emitFunction(acc->getStorage()->getAccessor(AccessorKind::Get));
    } else {
