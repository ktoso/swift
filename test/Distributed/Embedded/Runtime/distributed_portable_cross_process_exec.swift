...
//
// Build the ordinary-Swift (Codable/JSON) client and the Embedded server from
// the SAME source, sharing the same actor system:
...
//
// Phase 1 - the Codable client serializes the request (real JSON) to a file:
// RUN: %target-run %t/plain.out send %t/request.bin | %FileCheck %s --check-prefix=SEND
// Phase 2 - the Embedded server reads the request on stdin, dispatches through
//           the synthesized `_executeDistributedTarget`, writes the response on stdout:
// RUN: %target-run %t/embedded.out < %t/request.bin > %t/response.bin
// Phase 3 - the Codable client decodes the response the Embedded server produced:
// RUN: %target-run %t/plain.out recv %t/response.bin | %FileCheck %s --check-prefix=RECV
...
struct Main {
  static func main() async {
    ...
    do {
      ...
      // dispatches the incoming call. Under Embedded it forwards to the actor's
