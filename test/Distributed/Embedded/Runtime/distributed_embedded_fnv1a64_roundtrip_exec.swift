...
// RUN: %target-swift-frontend -target %target-cpu-apple-macos14 -enable-experimental-feature Embedded -enable-experimental-feature EmbeddedDistributed -distributed-id-gen=fnv1a-64 -parse-as-library -plugin-path %swift-plugin-dir %s %S/Inputs/ResolvableWorker.swift %S/Inputs/PortableRoundtripActorSystem.swift -c -o %t/a.o
...
// The numeric-identifier counterpart of distributed_embedded_roundtrip_exec.swift:
// the same round-trips must still dispatch correctly when target identifiers are
// FNV-1a 64 hashes (a flat numeric switch) instead of mangled thunk names. The
// second case also exercises the '@Resolvable' '$P'-proxy path, where the wire
// identifier is derived from the protocol-extension stub's mangled name.
