...
// AST-level shape of the compiler-synthesized `_executeDistributedTarget` under
// Embedded Swift - the receiver-side dispatch. We assert only the KEY nodes (the
// switch, the identifier compare, and the three calls that make a call happen);
// the detailed AST needed to make it work is intentionally not pinned here. The
// SIL / IR shapes are checked by the *_sil / *_irgen tests.
//
// In pseudo-Swift the synthesized method is:
//
