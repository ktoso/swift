...
distributed actor GreeterImpl: Greeter {
  ...
  distributed var currentSelf: any Greeter {
    // Resolve self as $Greeter so the fake roundtrip system gets a proper remote proxy
    // (a real Codable actor system would encode self.id and decode as $Greeter on the caller side)
