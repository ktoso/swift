...
func test_remote_invoke(address: ActorAddress, transport: ActorTransport) async {
  func check(actor: SomeSpecificDistributedActor) async {
    let personality = _isRemoteDistributedActor(actor) ? "remote" : "local"
    ...
    // error throws
    if _isRemoteDistributedActor(actor) {
      do {
  ...
  print("local isRemote: \(_isRemoteDistributedActor(local))")
  ...
  print("remote isRemote: \(_isRemoteDistributedActor(remote))")
