...
protocol ProgressHandler: AnyObject, UnsafeConcurrentValue {
...
extension Task {
  ...
  static func aggregateProgress<T>(
    ...
    return await Task.withLocal(\.progressHandler, boundTo: nil) {
      ...
      var aggregator = ProgressAggregator(totalUnitCount: aggregateTotalUnitCount, file: file, line: line, claimed: handler)
...
func preheatOven(temperature: Int) async -> Oven {
  await Task.reportProgress(pending: 8) { progress in
    for _ in 1...8 {
      progress.increment()
    }
    return .init()
